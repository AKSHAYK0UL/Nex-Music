...
class DownloadProvider {
  ...
  Stream<double> downloadSong(
    ...
    () async {
      try {
        ...
        if (!await File(metadataFilePath).exists()) {
          try {
            final safeJson = _toSafeJson(metadata);
            await File(metadataFilePath).writeAsString(jsonEncode(safeJson));
          } catch (e) {
            debugPrint("Failed to write metadata: $e");
          }
        }
  ...
  /// Convert Songmodel to safe JSON, especially for Timestamp handling
  Map<String, dynamic> _toSafeJson(Songmodel metadata) {
